...
TTY_NAMES=`ps aux | grep ${EXECUTABLE_FILENAME} | grep build/debug | grep -v '\.sh' | grep -v grep | awk '{print $7}'| sort | sed -e 's/^/RSS_/' | awk '{printf(",%s",$NF)}'`

echo "DATE$TTY_NAMES" > $HOME/test_runs/$RUN_DATE/${EXECUTABLE##*/}_memory.csv

while [ : ] ; do
	MEM=`ps aux | grep ${EXECUTABLE_FILENAME} | grep build/debug | grep -v '\.sh' | grep -v grep | awk '{print $7,$6}'|sort | awk '{printf(",%s",$NF)}'`
	DATE=`date '+%s'`
